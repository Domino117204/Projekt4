...
class Graph:
    ...
    def find_hamilton_cycle_heuristic(self):
        visited = set()
        path = []
        ...
        def greedy_dfs(node):
            path.append(node)
            visited.add(node)
            neighbors = sorted(self.adj[node], key=lambda x: len(self.adj[x]))
            for neighbor in neighbors:
                if neighbor not in visited:
                    if greedy_dfs(neighbor):
                        return True
            if len(path) == self.n and path[0] in self.adj[path[-1]]:
                path.append(path[0])
                return True
            visited.remove(node)
            path.pop()
...
def benchmark():
    ...
    for n in n_values_ham:
        ...
        print("  - Pomiar czasu cyklu Hamiltona (heurystyka)...")
        start = time.perf_counter()
        _ = g.find_hamilton_cycle_heuristic()
        elapsed = time.perf_counter() - start
    ...
    save_to_csv('euler_times_fast.csv', euler_times)
    save_to_csv('hamilton_times_fast.csv', hamilton_times)
    save_to_csv('nonham_hamilton_times_fast.csv', nonham_hamilton_times)
